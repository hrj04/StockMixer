...
sharpe5 = []
prec_10 = []
for i in range(len(best_test_perf_lst)):
    mse.append(best_test_perf_lst[i]['mse'])
    IC.append(best_test_perf_lst[i]['IC'])
    RIC.append(best_test_perf_lst[i]['RIC'])
    sharpe5.append(best_test_perf_lst[i]['sharpe5'])
    prec_10.append(best_test_perf_lst[i]['prec_10'])

print(f"mse: {np.mean(mse):0.3f} std: {np.std(mse):0.3f}")
print(f"IC: {np.mean(IC):0.3f} std: {np.std(IC):0.3f}")
print(f"RIC: {np.mean(RIC):0.3f} std: {np.std(RIC):0.3f}")
print(f"sharpe5: {np.mean(sharpe5):0.3f} std: {np.std(sharpe5):0.3f}")
print(f"prec_10: {np.mean(prec_10):0.3f} std: {np.std(prec_10):0.3f}")
sys.stdout.flush()